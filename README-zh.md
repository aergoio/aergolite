<p align="right"><a href="README.md">English</a>&nbsp;&nbsp;&nbsp;<a href="README-pt.md">Português</a>&nbsp;&nbsp;&nbsp;<a href="README-ru.md">Русский</a>&nbsp;&nbsp;&nbsp;<a href="README-ja.md">日本語</a>&nbsp;&nbsp;&nbsp;<a href="README-ko.md">한국어</a></p>
...
<h1 align="center">去中心化的SQLite复制</h1>

AergoLite提供了一个由私有轻量级区块链保护的可复制的SQLite数据库。

每个应用程序都有数据库的本地副本。

新的数据库交易将分发给所有对等方，一旦它们对执行交易达成共识，所有节点将执行交易。 由于执行顺序相同，因此所有节点都具有相同的结果。

应用程序也可以在脱机时写入本地数据库。 重新建立连接后，数据库交易将存储在本地队列中并发送到网络。 该应用程序将读取带有离线修改的数据库的新状态，并且它可以检查离线交易是否已由全局共识处理。 如果被拒绝，数据库将返回到先前的状态。
...
基本的复制解决方案允许节点在数据库上执行任何SQL命令。
它们不安全，因为获得单个节点控制权的攻击者可以删除和/或覆盖整个网络上的数据。

真正的去信任复制解决方案应该限制节点可以执行的操作，而不是信任它们。

攻击者可能能够修改本地数据库，但这不会反映在其他节点上。

对网络的成功攻击需要攻击者获得大多数节点的控制权。这种设计确保了强大的安全级别，使任何单个实体都难以破坏系统的完整性。


## 存储过程

AergoLite使用SQL编写的存储过程来控制节点可以执行的操作。

出于安全考虑：

1. 只有管理员可以创建存储过程。
2. 节点只能执行这些存储过程。其他SQL命令被阻止。
...
可用命令的完整参考可以在[这里](https://github.com/aergoio/sqlite-stored-procedures)找到

可以在同一网络上使用不同编程语言制作的应用程序，因为核心逻辑是通过存储过程完成的。


## 不可变性

如果您将存储过程限制为仅包含`INSERT INTO`命令，那么数据库内容将是不可变的。

但即使是允许更新和删除行的正常使用，更改的历史记录（所有SQL命令）都会记录在轻量级区块链上且无法删除，从而允许数据恢复和审计。
...
此版本使用完全连接的网络进行节点之间的通信。 它最多可与200个节点一起使用（通过自动化测试验证），可能更多。 将来，它还可以包含基于gossip的协议，以支持更多节点。
